...
interval = "2minute"  # You can use '5minute', '15minute', etc.
...
# symbol_degree_sorted = sorted(symbol_degree, key=lambda x: x['degree'], reverse=True)

# Print the sorted list
# print(symbol_degree_sorted)
...
def home():
    ...
    for symbol in symbols:
        ...
        try:
            data = kite.historical_data(
                ...
                from_date="2024-12-18 09:00:00",  # Adjust the date range
                to_date=to_date,
                interval=interval,
            )
            ...
            list_of_data = list_of_data[0:2]
            
            first_candle_open = list_of_data[0]['open']
            second_candle_close = list_of_data[1]['close']


            fc_low = list_of_data[0]['low']
            sc_high = list_of_data[1]['high']
            price_change= sc_high - fc_low

            time_difference =  2
            Vertical_Change = second_candle_close - first_candle_open
            slope = Vertical_Change/ time_difference   
            theta_radians = math.atan(slope)
            theta_degrees = math.degrees(theta_radians)

            if price_change < 20:
                symbol_degree.append({'symbol': symbol, 'degree': round(theta_degrees, 2),'change':round(price_change, 2) })
    ...
    return  render_template('index.html', symbol_degree_sorted=symbol_degree_sorted)
