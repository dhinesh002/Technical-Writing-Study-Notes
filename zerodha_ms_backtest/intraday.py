...
from_date = '2024-12-17 09:00:00'
to_date ='2024-12-18 15:29:00'
...
for symbol in symbols:
    ...
    try:
        data = kite.historical_data(    
            ...
            from_date="2024-12-17 09:00:00",  # Adjust the date range
            to_date=to_date,
            interval=interval,
        )
        ...
        if list_of_data[4]['EMA9'] > list_of_data[4]['EMA20']:
                
            for data in list_of_data:
                if data['EMA9'] < data['EMA20']:
                    if down_start == '':
                        down_start = data['date']
                
                if data['EMA9'] > data['EMA20']  and down_start != '':
                    up_start = data['date']                    
                    break
            filtered_data.append({f'symbol: {symbol}, down_start:{down_start}, up_start: {up_start}'})  
            # if down_start != '' and up_start != '':
                # print({f'symbol: {symbol}, down_start:{down_start}, up_start: {up_start}'})
            # print({f'symbol: {symbol}, down_start:{down_start}, up_start: {up_start}'})
